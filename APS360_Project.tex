...
Given the goal of the project is to train an neural network model to both recognize and classify images of Pokemon fed to it, we felt it was important to understand current models surrounding image recognition and classification in order to derive the best architecture of our model. Althought there are two popular machine learning architectures to choose from: multi-layer perceptrons (MLP) - which are simply fully connected ANN's or Convolutional Neural Networks (CNNs), much of the drawbacks of traditional MLP's lead it to being the suboptimal choice. Since image classification would require us to have to process every pixel in an image, many MLP models will end up having an overwhealming amount of parameters, which will greatly increase the models run time. Additionally, fully connected MLP's would also require us to preprocess every single data input, increasing the tediousness of data processing. 

In contrast, CNN's provide us with great amounts of flexibility in terms of our inputs, while not sacrificing the quality of predictions. CNN's operate with a logic that we do not need to process every single feature, and instead we can learn local features of an image and dectct these features throught an image. In other words, if we can learn features of an image, we can then use these features we learnt to recognize the image itself. Throught this weight sharing, the CNN can effectivley categorize images, which is exactly what we are looking for within our model. As a result, utilizing a CNN seems to be the best option we can utilize in order to effectivley create our model. 

CNN's themselves have many properties which we can also take advantage of when building our model. To start, we can take advantage of the AlexNet CNN architecture to use within our model. AlexNet is a commonly used CNN architecture, which is highly touted for its improvements over classical CNN's throught utilizing ReLu activiation functions and a very vast layer of neurons. We will also take advantage of one of the best featues of alexNet, which is its transfer learning ability as well. In short, we can import a pre-trained feature recognition alexNet CNN (through the alexNet.features module in PyTorch), and utilize these pre-trained features to classify our model. This cuts out a significant portion of our training of the model, and ensures accuracy throught alexNet's strong regularization and normalization of data. alexNet's implementation allows us to circumvent a previous issue which address with MLP, which would be having an overwhealming number of parameters, by providing us with a network which provides us with a large number of parameters without having to hand-encode them. This accomplishes the best of both worlds - providing us with a strong model with a large number of connections without creating a tedious amount of work. 
