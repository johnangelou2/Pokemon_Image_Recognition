...
Given the goal of the project is to train an neural network model to both recognize and classify images of Pokemon fed to it, we felt it was important to understand current models surrounding image recognition in order to develop a foundation into how our we will build our model. Image classification itself is traditionally done through Convolutional Neural Networks (CNNs). These are typically advantageous over normal multilayer perceptron architectures given that CNN's are capable of dealing with the multi-dimensionality of images and pixel calcualtion in an efficient way without inquiring large numbers of calculations. CNN's themselves also 
